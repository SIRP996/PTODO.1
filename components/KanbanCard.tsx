...
const KanbanCard: React.FC<KanbanCardProps> = ({ task, subtasks, onDragStart, isDragging, onToggleTaskUrgency, onDeleteTask, onStartFocus, onToggleTask, onUpdateTaskNote }) => {
  ...
  return (
    <div
      ...
      className={`bg-slate-800 p-4 rounded-lg shadow-md cursor-grab active:cursor-grabbing border border-slate-700/50 transition-all ${isDragging ? 'opacity-50' : 'opacity-100'} ${task.isUrgent && task.status !== 'completed' ? 'border-l-4 border-red-500' : ''}`}
    >
